...
class ReportWriter(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            HealthAlert,
            "/kestrel/health_alerts",
            self.on_health_alert,
            qos_profile_sensor_data,
        )
        self.create_subscription(
            ObstacleHazard,
            "/kestrel/obstacle_hazards",
            self.on_obstacle_hazard,
            qos_profile_sensor_data,
        )
        self.create_subscription(
            NavigationAdvisory,
            "/kestrel/nav_advisories",
            self.on_nav_advisory,
            qos_profile_sensor_data,
        )
        self.create_subscription(
            String,
    ...
    def write_report(self):
        ...
        if findings_text is not None:
            model_used = self.llm_model or PROVIDER_DEFAULT_MODELS.get(
                self.llm_provider
            )
