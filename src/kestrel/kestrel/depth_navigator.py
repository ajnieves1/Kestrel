...
class DepthNavigator(Node):
    ...
    def on_image(self, image_message):
        ...
        hazard, heading, center, clearest = analyze_depth(depth, self.hazard_ratio)
    ...
    def raise_advisory(self, heading, center, clearest):
        ...
        advisory.message = (
            f"structure close ahead, more clearance to the {heading}, recommend veering"
        )
        self.advisory_publisher.publish(advisory)
        self.get_logger().warn(f"navigation advisory: structure ahead, veer {heading}")
