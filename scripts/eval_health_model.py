...
def main():
    ...
    for path, (kind, errors, times) in per_log.items():
        ...
        lines.append(
            f"| {os.path.basename(path)} | {kind} | " + " | ".join(cells) + " |"
        )
    lines.append("")

    # Per window ROC AUC
    nominal_errors = np.concatenate([per_log[p][1] for p, k in logs if k == "nominal"])
    fault_errors = np.concatenate([per_log[p][1] for p, k in logs if k == "fault"])
