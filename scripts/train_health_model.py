...
class Autoencoder(nn.Module):
    def __init__(self, input_size):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_size, 16), nn.ReLU(), nn.Linear(16, 8), nn.ReLU()
        )
        self.decoder = nn.Sequential(
            nn.Linear(8, 16), nn.ReLU(), nn.Linear(16, input_size)
        )
...
def reconstruction_error(model, windows):
    with torch.no_grad():
        ...
        return ((reconstructed - torch.from_numpy(windows)) ** 2).mean(dim=1).numpy()
...
def main():
    ...
    active_index = [
        i for i in range(len(FEATURE_COLUMNS)) if feature_std[i] > DEAD_FEATURE_STD
    ]
    ...
    print(
        f"active features: {active_names} (dropped "
    ...
    for samples in nominal_files:
        ...
        standardized[:, active_index] = (samples[:, active_index] - means) / stds
    ...
    print(
        f"nominal error: mean {nominal_errors.mean():.6f} "
...
def offline_eval(active_index, means, stds, threshold):
    ...
    for path in sorted(glob.glob(os.path.join(LOG_DIRECTORY, "*.csv"))):
        ...
        print(
            f"  {os.path.basename(path):40s} {fraction * 100:6.1f}%  ({count} windows)"
        )
