...
class TelemetryRecorder(Node):
    ...
    def sample(self):
        ...
        ang_vel_mag = math.sqrt(angular.x**2 + angular.y**2 + angular.z**2)
        lin_acc_mag = math.sqrt(linear.x**2 + linear.y**2 + linear.z**2)
    ...
    def close(self):
        ...
        self.get_logger().info(f"wrote {self.row_count} rows to {self.output_path}")
